//making a (slightly? improved version of our code)
...
template<typename T> size_t closest_point_convert(const point<T>& p, sequence<center<double>>& centers) {
    if constexpr(std::is_same<T,double>() == true) {
        ...
        return closest_point(p,centers);
    }
    else {
...
template <typename T> std::pair<sequence<center<double>>,double> kmeans_doublecenter(parlay::sequence<point<T>>& pts, parlay::sequence<center<double>>& centers, size_t k, size_t max_iterations, double epsilon){
